...
static int configure_default_ssl(sslclient_context *ssl_client) {
  ...
  return mbedtls_ssl_config_defaults(&ssl_client->ssl_conf,
 ...
 * \return int          0 if successful, +int num certs failed, -int X509 or PEM error code.
 */
...
static int configure_client_cert_key(sslclient_context *ssl_client, const char *cli_cert, const char *cli_key) {
  ...
  log_v("Setting own certification chain");
 ...
 * \return int        0 if successful.
 */
int perform_handshake(sslclient_context *ssl_client, const char *host, int timeout) {
  int ret = -1;
...
/**
 * \brief             Confirm the protocols and ciphersuite used for the connection.
 * 
 * \param ssl_client  sslclient_context* - The ssl client context.
 * \param cli_cert    const char* - The client certificate. 
 * \param cli_key     const char* - The client key. 
...
//   if (initialize_ssl_client(ssl_client, host, port) != 0) {
//     return -1;
//   }

//   if (seed_rng(ssl_client) != 0) {
//     return -2;
//   }

//   if (setup_ssl_configuration(ssl_client, rootCABuff, cli_cert, cli_key, pskIdent, psKey) != 0) {
//     return -3;
//   }

//   if (load_certificates_and_keys(ssl_client, rootCABuff, cli_cert, cli_key) != 0) {
//     return -4;
//   }
  
//   if (perform_handshake(ssl_client, host) != 0) {
//     return -5;
//   }

//   confirm_protocols(ssl_client, cli_cert, cli_key);
  
//   if (verify_peer_certificate(ssl_client, rootCABuff, cli_cert, cli_key) != 0)  {
//     return -6;
//   }
